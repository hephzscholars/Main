jQuery(document).ready(function( $ ) {
  ...
  function countUp(el, target) {
    ...
    const interval = setInterval(() => {
      ...
      el.textContent = count.toLocaleString(); // Format number with commas
  ...
  // Start counting when the page loads and when scrolling
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function checkCounters() {
      if (!hasCounted && isElementInViewport(document.getElementById("facts"))) {
        counters.forEach((counter) => {
          const target = parseInt(counter.textContent.replace(/,/g, ''), 10);
          countUp(counter, target);
        });
        hasCounted = true; // Prevent further counting
      }
    }
